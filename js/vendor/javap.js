...
(function () {
    ...
    function ClassFileParser(binary) {
        ...
        function getStackMap(attr) {
            ...
            len = attr.numberOfEntries;
            for (i = 0; i < len; i++) {
                entries.push(getStackMapEntry());
            }

            attr.entries = entries;
        }

        function getStackMapEntry() {
            var entry = {};
...
//      verification_type_info
    ...
    function parseLookupswitch(code, index) {
        ...
        while (defaultByteIndex % 4 !== 0) {
            operand.push(code[defaultByteIndex]);
            defaultByteIndex++;
        }

        operand.push(code[defaultByteIndex+0]);
        operand.push(code[defaultByteIndex+1]);
        operand.push(code[defaultByteIndex+2]);
        operand.push(code[defaultByteIndex+3]);
        operand.push(code[defaultByteIndex+4]);
        operand.push(code[defaultByteIndex+5]);
        operand.push(code[defaultByteIndex+6]);
        operand.push(code[defaultByteIndex+7]);
    ...
    JVM.ByteCodeParser.parse = function (code) {
        ...
        for (i = 0; i < len; i++) {
            try {
                ...
                byteCode = bytecode('unknown:'+code[i], 0).parse(code, i);
                //throw e;
            }
